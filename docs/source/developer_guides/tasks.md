...
- [`KkTestTask`](../autoapi/czbenchmarks/tasks/single_cell/kk_test/index): Evaluates embeddings on rare cell type detection by identifying rare cell types based on frequency thresholds and assessing classifier performance using stratified k-fold cross-validation. Computes metrics including F1 score, precision, recall, balanced accuracy, MCC, specificity, and per-type performance metrics.
